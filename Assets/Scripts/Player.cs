...
public class Player : MonoBehaviour
{
    ...
    void Update() {
        ...
        //apuntar als enemics enviant raig desde la posició del player. Així no espantarà a través de parets

        if(Physics.Raycast(transform.position, transform.forward, out RaycastHit hit, 40f)) {

            //Debug.DrawRay(transform.position, transform.forward * 40f, Color.red);
            ...
            if(hit.collider.tag == "Guard" || hit.collider.tag == "Worker") 
            {
                ...
                enemy.GetComponentInChildren<EnemyAI>().currentState.ScareEnemy(); //activar barra por    
            } 
        }
    ...
    private void OnTriggerEnter(Collider other) 
    {
        ...
        if(other.tag == "Worker" || other.tag == "Guard") 
        {
